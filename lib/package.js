const ar = require('ar-async'),
    ...
    hasProperty = require('./util/property');
...
(function () {
    ...
    function Packager(options) {
        ...
        this.minify = true;
        if (options && hasProperty(options, 'minify')) {
            this.minify = options.minify;
        }
        this.excludeFiles = [];
        if (options && hasProperty(options, 'excludefiles')) {
        ...
        this.rom = false;
        if (options && hasProperty(options, 'rom')) {
            this.rom = options.rom;
        }

        this.encrypt = false;
        if (options && hasProperty(options, 'encrypt')) {
            this.encrypt = options.encrypt;
        }

        this.sign = "";
        if (options && hasProperty(options, 'sign')) {
            this.sign = options.sign;
        }

        this.certificate = "";
        if (options && hasProperty(options, 'certificate')) {
        ...
        this.pkgVersion = options.pkgversion || "0.0.1";

        if (options && hasProperty(options, 'pkgid')) {
            this.pkgId = options.pkgid;
        }

        if (options && hasProperty(options, 'pkginfofile')) {
        ...
        analyzeIPK: function (options, next) {
            ...
            function _setConfig(next) {
                ...
                if (options.info === 'true') {
                    return next(errHndl.changeErrMsg("EMPTY_VALUE", 'info'));
                } else if (options.infodetail === 'true') {
                    return next(errHndl.changeErrMsg("EMPTY_VALUE", 'info-detail'));
                }
                ...
                if (path.extname(ipkFile) !== ".ipk") {
                    return next(errHndl.changeErrMsg("SUPPORT_ONLY_IPK", ipkFile));
                }
                if (!fs.existsSync(ipkFile)) {
                    return next(errHndl.changeErrMsg("NOT_EXIST_PATH", ipkFile));
                }
                if (!fs.existsSync(ipkConfigFile)) {
                    return next(errHndl.changeErrMsg("NOT_EXIST_PATH", ipkConfigFile));
                }
            ...
            function _unpackTar(next) {
                ...
                if (fs.existsSync(path.resolve(tmpDirPath, "control.tar.gz"))) {
                    ...
                    return next(errHndl.changeErrMsg("NOT_SUPPORT_XZ"));
                } else {
                    return next(errHndl.changeErrMsg("NO_COMPONENT_FILE", ", control tar file"));
                }
                ...
                if (fs.existsSync(path.resolve(tmpDirPath, "data.tar.gz"))) {
                    ...
                    return next(errHndl.changeErrMsg("NO_COMPONENT_FILE", "data tar file"));
                }
            }
                ...
                ipkConfig.webOSMetaFiles.forEach(function (item) {
                    ...
                    if (item === "control") {
                        ...
                        if (!fs.existsSync(targetFile)) {
                            return next(errHndl.changeErrMsg("NO_COMPONENT_FILE", "control file"));
                        }
    ...
    function loadPkgInfo(next) {
        ...
        if (fs.existsSync(this.pkginfofile)) {
            ...
            try {
                ...
                if (!hasProperty(this.pkginfo, 'id')) {
    ...
    function loadAppInfo(next) {
        ...
        const data = rewriteFileWoBOMAsUtf8(filepath, true);
        try {
    ...
    function checkAppInfo(next) {
        ...
        if (!this.appinfo.version || this.appinfo.version === undefined) {
            return setImmediate(next, errHndl.changeErrMsg("REQUIRED_FIELD", "version"));
        }
    ...
    function fillAssetsField(next) {
        ...
        for (const i in this.appinfo) {
            if (hasProperty(this.appinfo, i) && defaultAssetsFields[i]) {
        ...
        const checkDir = path.join(this.originAppDir, "resources");
        const foundFilePath = [];
                    ...
                    rewriteFileWoBOMAsUtf8(filePath, true, function (err, data) {
                        try {
                            ...
                            for (const i in appInfo) {
                                if (hasProperty(appInfo, i) && defaultAssetsFields[i]) {
                                    if (appInfo[i]) {
                                        const itemPath = path.join(dirPath, appInfo[i]);
                                        const relPath = path.relative(appInfoPath, itemPath);
                                        // no duplicated adding & value should not null string & file/dir should exist
    ...
    function copyAssets(next) {
        ...
        function _handleAssets(file, next) {
            log.verbose("copyAssets():_handleAssets(): ", file);
    ...
    function excludeIpkFileFromApp(next) {
        ...
        // Exclude pre-built *.ipk files
        // Exclude .reloadignore file
    ...
    function excludeFromApp(next) {
        ...
        const strRegExp = regExpQueries.join("|");
        const regExp = new RegExp(strRegExp, "i");
        const excludeList = [];
    ...
    function fillPackageDir(next) {
        ...
        if (!this.rom) {
            ...
            if (!this.pkgDir) {
                let data = "";
                if (this.pkginfo) {
                    if (!hasProperty(this.pkginfo, 'version')) {
    ...
    function outputPackage(destination, next) {
        ...
        if (this.rom) {
            ...
            const tempCtrlDir = path.join(tempDir, 'ctrl');
            const ctrlTgzFile = path.join(tempDir, 'control.tar.gz');
            const tempDataDir = path.join(tempDir, 'data');
            const dataTgzFile = path.join(tempDir, 'data.tar.gz');
    ...
    function createSign(dstDir, dataTgzPath, next) {
        ...
        const keyPath = path.resolve(this.sign);
        const crtPath = path.resolve(this.certificate);
        ...
        try {
            ...
            const dataFile = fs.readFileSync(dataTgzPath); // data.tar.gz
    ...
    function makeIpk(srcDir, next) {
        ...
        const header = "!<arch>\n";
        const debBinary = arFileHeader("debian-binary", 4) + "2.0\n";
        const that = this;
        const arStream = CombinedStream.create();
        const pkgFiles = ['control.tar.gz', 'data.tar.gz'];
        const ipkStream = fstream.Writer(this.ipk);

        arStream.append(header + debBinary);

        pkgFiles.forEach(function (f) {
            const fpath = path.join(that.IpkDir, f);
            const s = fstream.Reader({ path: fpath, type: 'File' });
            const stat = fs.statSync(fpath); // TODO: move to asynchronous processing
    ...
    function loadServiceInfo(next) {
        ...
        for (const idx in this.services) {
            ...
            try {
                ...
                if (!(hasProperty(info, 'id') && hasProperty(info, 'services'))) {
                    continue;
                }
    ...
    function copyService(next) {
        ...
        const self = this;
        const validServices = this.services.filter(function (service) {
    ...
    function encryptPackage(next) {
        if (this.rom || !this.encrypt) { // Do not encrypt when -rom option is given
            ...
            const encryptCtrlDir = path.join(encryptDir, 'ctrl');
            const ctrlTgzFile = path.join(encryptDir, 'control.tar.gz');
            const encryptDataDir = path.join(encryptDir, 'data');
            const dataTgzFile = path.join(encryptDir, 'data.tar.gz');
    ...
    function encryptIpk(dstPath, next) {
        ...
        const encrypedIpkPath = path.join(dstPath, this.ipkFileName);
